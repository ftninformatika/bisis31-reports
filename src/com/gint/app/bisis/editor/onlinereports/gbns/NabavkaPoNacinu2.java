package com.gint.app.bisis.editor.onlinereports.gbns;
...
public class NabavkaPoNacinu2 implements OnlineReport {

  public String getName() {
    return "NabavkaPoNacinuZaPeriod.xml";
  }
  
  public String run(String recordsFileName, Map params) {
    String sStartDate = (String)params.get("startDate");
    String sEndDate = (String)params.get("endDate");
    try {
      startDate = intern.parse(sStartDate);
      endDate = intern.parse(sEndDate);
    } catch (Exception ex) {
      return "error: invalid date(s) " + sStartDate + ", " + sEndDate;
    }
    
    RandomAccessFile records = null;
    try {
      records = new RandomAccessFile(recordsFileName, "r");
    } catch (Exception ex) {
      return "error: cannot open file " + recordsFileName;
    }
    ...
    report.append(sdf.format(startDate) + " - " + sdf.format(endDate));
    report.append("\">\n");
